...
updatedAt: 1780368804536
---

# yooasset-resource-manager

## Summary
YooAsset 桥接资源管理层的位置、初始化方式、主要接口；SceneManager Core Loading 当前通过 AssetsManager 加载并用 SceneHandle 卸载场景；UIResourceManager 当前通过 AssetsLoadManager 加载 UI prefab；烟测包含资源泄露循环检测。
...
- `AssetsManager` 主资源接口包括同步/异步加载��源、子资源加载、`LoadSceneAsync`、`UnloadSceneAsync(SceneHandle)`、`RemoveUnusedAssets`。
